...
def test_square_image_is_saved_as_jpeg(tmp_path):
    """A 100x100 PNG is saved as _square.jpg in the cropped directory."""
    cover_dir = tmp_path / "covers"
    cropped_dir = tmp_path / "cropped"
    cover_dir.mkdir()
    _create_test_image(str(cover_dir / "thumb.png"), 100, 100)
    crop_thumbnail_to_square("thumb", str(cover_dir), str(cropped_dir))

    out = cropped_dir / "thumb_square.jpg"
    assert out.exists()
...
def test_landscape_image_is_cropped_to_square(tmp_path):
    ...
    cropped_dir = tmp_path / "cropped"
    cover_dir.mkdir()
    _create_test_image(str(cover_dir / "wide.png"), 200, 100)
    crop_thumbnail_to_square("wide", str(cover_dir), str(cropped_dir))

    out = cropped_dir / "wide_square.jpg"
    assert out.exists()
    with Image.open(str(out)) as img:
        assert img.size == (100, 100)
...
def test_portrait_image_is_cropped_to_square(tmp_path):
    ...
    cropped_dir = tmp_path / "cropped"
    cover_dir.mkdir()
    _create_test_image(str(cover_dir / "tall.png"), 100, 200)
    crop_thumbnail_to_square("tall", str(cover_dir), str(cropped_dir))

    out = cropped_dir / "tall_square.jpg"
    assert out.exists()
    with Image.open(str(out)) as img:
        assert img.size == (100, 100)
...
def test_rgba_is_converted_to_rgb(tmp_path):
    ...
    cropped_dir = tmp_path / "cropped"
    cover_dir.mkdir()
    _create_test_image(str(cover_dir / "alpha.png"), 150, 100, mode="RGBA")
    crop_thumbnail_to_square("alpha", str(cover_dir), str(cropped_dir))

    out = cropped_dir / "alpha_square.jpg"
    assert out.exists()
    with Image.open(str(out)) as img:
        assert img.mode == "RGB"
...
def test_missing_thumbnail_does_not_raise(tmp_path):
    ...
    crop_thumbnail_to_square("nonexistent", str(tmp_path), str(tmp_path / "cropped"))
...
def test_webp_thumbnail_is_found_and_cropped(tmp_path):
    ...
    cropped_dir = tmp_path / "cropped"
    cover_dir.mkdir()
    _create_test_image(str(cover_dir / "track.webp"), 300, 200)
    crop_thumbnail_to_square("track", str(cover_dir), str(cropped_dir))

    out = cropped_dir / "track_square.jpg"
    assert out.exists()
    with Image.open(str(out)) as img:
